...
### Step 2 — Risk-based UI decision

Classify UI evidence first. Skip E2E for `not_applicable` or `static_pass`; run live Playwright for
`playwright_required`; block material `blocked_runtime_evidence`. `vspec-mock` is opt-in simulation
only and cannot satisfy runtime evidence. Continue static verification after the UI decision.
