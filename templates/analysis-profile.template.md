...
- **`verify_policy`**: `risk-based` (default). Static spec-vs-code comparison is independent of
  Playwright. Run browser verification only for JavaScript/AJAX/download/layout/runtime signals.
  A material runtime fact without a runnable environment blocks; Mock remains simulation only.
